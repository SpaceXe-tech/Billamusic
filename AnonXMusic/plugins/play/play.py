...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            # Enhanced Apple Music support while maintaining old structure
            if "track" in url or "/song/" in url or "?i=" in url:
                ...
                try:
                    ...
                    return await mystic.edit_text(_["play_3"])
                streamtype = "youtube"
                # Use Apple artwork if available, fallback to regular thumb
                img = details.get("apple_artwork") or details["thumb"]
                # Use Apple title if available, fallback to regular title
                title = details.get("apple_title") or details["title"]
                cap = _["play_10"].format(title, details["duration_min"])
            elif "album" in url:
                # Handle albums - try enhanced method first, fallback to simple
                try:
                    ...
                    # Fallback to single track method like old version
                    try:
                        details, track_id = await Apple.track(url)
                        streamtype = "youtube"
                        img = details.get("apple_artwork") or details["thumb"]
